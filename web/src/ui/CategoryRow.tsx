...
export function CategoryRow({ ckey, ev, delta }: { ckey: CategoryKey; ev: Evaluation; delta: number | null }) {
  ...
  return (
    <div className="cat">
      ...
      <style>{`
        .cat{display:grid;grid-template-columns:minmax(0,1fr) 150px;gap:18px;align-items:center;padding:16px 2px;border-top:1px solid var(--rule)}
        .cat:first-child{border-top:none}
        .cat-name{font-size:12px;letter-spacing:.06em;font-weight:500}
        .cat-ev{color:var(--ink-soft);font-size:13px;margin-top:3px;max-width:80ch;overflow-wrap:anywhere}

...
export function ThemeToggle() {
  ...
  return (
    ...
    >
      ...
      <style>{`
        .ha-theme{position:relative;width:60px;height:28px;border-radius:999px;border:1px solid var(--rule);background:var(--panel-2);cursor:pointer;padding:0;flex:none;transition:border-color .15s ease}
        .ha-theme:hover{border-color:var(--ink-soft)}
        ...
        @media (prefers-reduced-motion: reduce){ .ha-knob,.ha-theme{transition:none} }
      `}</style>
    </button>
  );
}
